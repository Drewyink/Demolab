...
function crudRoutes(app, route, table, pgInsert, pgUpdate){
  ...
  app.post(`/api/${route}`, async (req, res) => {
    try { if (usePostgres) { const r = await pgInsert(req.body); return res.status(201).json(r); } res.status(201).json(memCreate(table, req.body)); } catch (e) { console.error(`POST /api/${route}:`, e.message); res.status(500).json({ error: e.message }); }
  });
  app.put(`/api/${route}/:id`, async (req, res) => {
    try { if (usePostgres) { const r = await pgUpdate(req.params.id, req.body); return r ? res.json(r) : res.status(404).json({ error: 'Not found' }); } const rec = memUpdate(table, req.params.id, req.body); rec ? res.json(rec) : res.status(404).json({ error: 'Not found' }); } catch (e) { console.error(`PUT /api/${route}:`, e.message); res.status(500).json({ error: e.message }); }
  });
  app.delete(`/api/${route}/:id`, async (req, res) => {
    try { if (usePostgres) { await pgQuery(`DELETE FROM ${table} WHERE id=$1`, [req.params.id]); return res.json({ success: true }); } memDelete(table, req.params.id) ? res.json({ success: true }) : res.status(404).json({ error: 'Not found' }); } catch (e) { res.status(500).json({ error: e.message }); }
  });
}
...
app.get('/api/stats', async (req, res) => {
  try {
    ...
    res.json({ members: db.members.length, products: db.products.length, plans: db.plans.length, benefits: db.benefits.length, pricing: db.pricing.length, claims: db.claims.length, groups: db.groups.length, providers: db.providers.length, authorizations: db.authorizations.length, total_claims_amount: totalBilled.toFixed(2), pending_claims: db.claims.filter(c=>c.status==='pending').length, active_members: db.members.filter(m=>m.status==='active').length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
