...
// ── SETUP (generates hashed users on first run) ───────────────
app.get('/api/setup', async (req, res) => {
  try {
    const hash = await bcrypt.hash('password', 10);
    await pool.query(`UPDATE users SET password_hash=$1`, [hash]);
    res.json({ message: 'All user passwords reset to: password' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/dashboard/stats', authenticateToken, async (req, res) => {
  try {
    const [members, products, plans, claims, pendingClaims, claimsTotal] = await Promise.all([
      ...
      pool.query('SELECT COALESCE(SUM(amount),0) as total FROM claims'),
    ]);
    const recentActivity = await pool.query(
      `SELECT al.*, u.full_name FROM audit_logs al LEFT JOIN users u ON al.user_id=u.id ORDER BY al.created_at DESC LIMIT 10`
    );
    res.json({
      stats: {
...
app.put('/api/members/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE members SET first_name=$1,last_name=$2,email=$3,phone=$4,date_of_birth=$5,gender=$6,address=$7,city=$8,state=$9,zip=$10,plan_id=$11,employer=$12,effective_date=$13,termination_date=$14,is_active=$15,updated_at=NOW()
...
app.get('/api/pricing', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT pr.*, p.name as plan_name FROM pricing pr LEFT JOIN plans p ON pr.plan_id=p.id ORDER BY p.name, pr.tier`
    );
    res.json(result.rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/claims', authenticateToken, async (req, res) => {
  try {
    ...
    let query = `SELECT c.*, m.first_name||' '||m.last_name as member_name, m.member_id as member_number, p.name as plan_name
                 FROM claims c LEFT JOIN members m ON c.member_id=m.id LEFT JOIN plans p ON c.plan_id=p.id WHERE 1=1`;
    const params = [];
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE claims SET status=$1,paid_amount=$2,allowed_amount=$3,denial_reason=$4,notes=$5,updated_at=NOW(),
       processed_date=CASE WHEN $1 IN ('approved','denied','paid') THEN NOW() ELSE processed_date END
